manhattan <- function (x, chr = "CHR", bp = "POS", p = "P", snp = "SNP", col = c("gray10",
    "gray60"), chrlabs = NULL, highlight = NULL, logp = TRUE,
    ...)
{
    ...
    def_args <- list(yaxt="n", xaxt = "n", bty = "n", xaxs = "i", yaxs = "i",cex.lab=2,cex.axis=1.5,
        las = 1, pch = 20, xlim = c(xmin, xmax), ylim = c(1.5,
    ...
    if (nchr == 1) {
        ...
        icol = 1
        for (i in unique(d$index)) {
            with(d[d$index == unique(d$index)[i], ], rect(xleft=min(pos),xright=max(pos),ybottom=2,ytop=ceiling(max(d$logp)),col = bg_col[icol], border=NA,...))
            icol = icol + 1
        }
        icol = 1
        for (i in unique(d$index)) {
    ...
    if (nchr == 1) {
        ...
        with(d.highlight, points(pos, logp, col = "firebrick1", pch = 20,
            ...))
    }
}
